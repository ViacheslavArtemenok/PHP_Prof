/*if (localStorage.getItem("someText")) {
  let arr_get = JSON.parse(localStorage.getItem("someText"));
  document.getElementById("hell_name").innerHTML = arr_get[0];
  document.getElementById("hell_age").innerHTML = arr_get[1];
}

function hello(id_get1, id_n, id_a, bbb) {
  let val1 = document.getElementById(id_get1).value;
  let arr_set = val1.split(",");
  console.log(arr_set + " " + bbb);
  localStorage.setItem("someText", JSON.stringify(arr_set));
  let arr_get = JSON.parse(localStorage.getItem("someText"));
  document.getElementById(id_n).innerHTML = arr_get[0];
  document.getElementById(id_a).innerHTML = arr_get[1];
  document.getElementById(id_get1).value = "Hello, " + arr_get[0] + "!";
}
function close_menu() {
  document.getElementById("header_menu").open = false;
  localStorage.clear();
  document.getElementById("hell_name").innerHTML = "";
  document.getElementById("hell_age").innerHTML = "";
}
*/
//let count = 3;
    ...
    arr_set.forEach((item, i) => {
      ...
      document.getElementById("get_cart").insertAdjacentHTML(
        ...
        `<div class="cart_change_item">
                <a href="catalog.php?type=${item.gend}"><img class="img_cart" src="img/${item.photo_name}" alt="product"></a>
...
function addToCart(gend, id, name, size, price, photo_name, quantity) {
  let arr_set = [];
  if (localStorage.getItem("cart")) {
...
/*function sendOrder() {
  ...
  xhttp.setRequestHeader("name", "order");
  xhttp.send(data);
}
